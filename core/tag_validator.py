...
def _check_block_resonance_balance(block_children: list) -> str:
    ...
    # 不是不同的共振式（守恒查不出——分子式当然相同）。实测病例：两个标着
    # 不同 label 的极限式 SMILES 逐字相同。
    # 只用逐字比对、不用 canonical SMILES：canonical 化会按图同构合并
    # 羧酸根 [O-]C=O ↔ O=C[O-] 这类"负电荷移到对称等价原子"的合法共振式
...
def _check_ring_claim_label(mol, label: str) -> str:
    ...
    - 「环状/环形 + 卤鎓」：卤鎓离子（溴鎓/氯鎓/碘鎓）在教学语境必为
      ...
      （实测病例：label 写"环状溴鎓离子"，SMILES 实为 Br+3C 四元环）。
      氧鎓/硫鎓环尺寸多样（环氧鎓三元、吡喃鎓六元）不纳入；「开环」等含"环"但非环状声明的措辞、无环状声明的鎓
